package com.codeyyt.gulimall.search.service.impl;
...
import com.codeyyt.gulimall.search.feign.ProductFeignService;
import com.codeyyt.gulimall.search.service.MallSearchService;
import com.codeyyt.gulimall.search.vo.AttrResponseVo;
import com.codeyyt.gulimall.search.vo.BrandVo;
    ...
    private SearchResponseVo buildSearchResult(SearchResponse response, SearchParamVo param) {
        ...
        if(param.getAttrs() != null && param.getAttrs().size() > 0){
            ...
            List<SearchResponseVo.NavVo> collect = param.getAttrs().stream().map(attr -> {
                ...
                navVo.setLink("localhost:88/search/list.html?"+ replace);
                return navVo;
            }).collect(Collectors.toList());
